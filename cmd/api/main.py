...
def provide_assist_graph():
    return get_assist_graph()
...
app = FastAPI(title="opencode-orchestrator", version="0.1.9", lifespan=lifespan)
...
def healthz() -> dict:
    ...
    return {
        ...
        "default_generation_provider": settings.inference.provider,
        "default_retrieval_provider": "qdrant",
...
def agentcore_config() -> dict:
    ...
    snapshot = adapter.snapshot_state() if hasattr(adapter, "snapshot_state") else {}
    return {
...
def providers() -> dict:
    ...
    return {
        "default_generation_provider": settings.inference.provider,
        "supported_generation_providers": ["openrouter", "bedrock_runtime"],
        "default_retrieval_provider": "qdrant",
        "supported_retrieval_providers": ["qdrant", "bedrock_knowledge_base"],
        "bedrock_enabled": settings.bedrock.enabled,
        "bedrock_runtime_model_id": settings.bedrock.runtime_model_id,
        "bedrock_knowledge_base_id": settings.bedrock.knowledge_base_id,
        "bedrock_knowledge_model_id": settings.bedrock.knowledge_model_id,
        "aws_region": settings.aws.region,
    }
...
def retrieve(payload: RetrieveRequest) -> RetrieveResponse:
    ...
    result = router.retrieve(
        ...
        model_arn_or_id=payload.model_arn_or_id,
    )
...
async def assist(
    ...
    graph=Depends(provide_assist_graph),
) -> AssistResponse:
    trace_id = request.state.trace_id
