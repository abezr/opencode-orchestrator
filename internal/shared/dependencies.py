...
from internal.platform.agentcore.gateway_mcp_client import AgentCoreGatewayMCPClient
...
def get_agentcore_adapter():
    ...
    if settings.agentcore.mode == "gateway_mcp":
        fallback = stub if settings.agentcore.fallback_to_stub_on_error else None
        return AgentCoreGatewayMCPClient(settings.agentcore, fallback_adapter=fallback)
    return stub


@lru_cache(maxsize=1)
def get_support_action_service() -> SupportActionService:
    return SupportActionService(settings=get_settings(), agentcore=get_agentcore_adapter())
